...
{self.YELLOW}[03]{self.RESET}{self.MAGENTA}help{self.RESET}\t{self.YELLOW}[04]{self.MAGENTA}exit{self.RESET}
 ...
 ''')
        ...
        if self.choice == "1" or "01":
            ...
            os.system(f'nmap -sC -sV -T4 {self.target} -oN {self.target}.result')
...
''')
            ...
            try : 
                ...
                os.system(f'nmap -sC -sV -T4 {command} -oN {command}.result')
            except:
            ...
            try : 
                command = command.split(' ')[1]
                os.system(f'wpscan --url {command}  --random-user-agent -o {command}.result')
            except:
                print(f'{self.CYAN}[{self.RED}~{self.GREEN}]{self.RED}Target not found !!!')
                print(f'{self.CYAN}[{self.YELLOW}?{self.GREEN}]{self.YELLOW}Target : {self.RESET}',end='')
                command = input()
